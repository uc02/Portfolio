  ...
  return (
    <div className='w-full h-screen flex flex-col justify-center items-center isolate'>
      ...
      <div className='flex flex-col md:items-center items-start xl:gap-y-10 gap-y-3 xl:mb-0 md:mb-20 mb-0'>
        ...
        <button className='xl:w-[400px] md:w-[300px] w-[270px] bg-gray-200 md:py-1 py-0 md:px-4 px-2 xl:text-xl  md:text-xl text-base text-gray-900 tracking-widest rounded-r-4xl flex justify-between items-center md:mr-auto md:mx-0 mx-auto' 
         ...
         <div className='flex md:gap-12 gap-2 mr-auto'>
          {socialIcons.map((social,index) => (
            <a 
             href=""
             key={index}
             className='xl:text-3xl md:text-2xl text-yellow-500 hover:text-white transition-colors duration-300'
            >
        ...
        <div 
         className='lg:w-[600px] md:w-[500px] w-[360px] absolute left-1/2 -translate-x-1/2 -z-10'
          >
          <img 
           src="images/road.png" 
           alt="Road Image" 
           ...
           <span className='xl:text-xs md:text-[10px] text-[8px] font-bold tracking-wide text-yellow-500 absolute -top-5 xl:right-22 lg:right-26 md:right-16 right-10 rotate-[3.5deg] animate-bounce'>
            Looking for new challenges
           </span>

          <div 
            className={`xl:h-[150px] h-[100px] px-3 xl:text-lg md:text-base text-xs font-light text-gray-200 text-justify overflow-y-auto tracking-wide origin-top custom-scrollbar
            ${isTextVisible ? 'scale-y-100' : 'scale-y-0'}
             transition-transform duration-300`}
             >
            <p 
              className='xl:py-3 py-1 px-1 [&::first-letter]:text-[30px] [&::first-letter]:ml-5 [&::first-letter]:text-yellow-500'>
               {aboutText}
            </p>
          </div>
        </div>
